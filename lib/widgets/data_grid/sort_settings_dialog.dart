...
class SortSettingsDialog extends HookWidget {
  ...
  /// Opens the sort settings dialog and returns the updated [List<SortColumnConfig>]
  ...
  static Future<List<SortColumnConfig>?> show(
    ...
    return showDialog<List<SortColumnConfig>>(
      context: context,
      builder: (context) => SortSettingsDialog(initialConfigs: initialConfigs),
    );
  }

  @override
  Widget build(BuildContext context) {
    // Local deep copy of configs — changes are only applied on "Übernehmen"
    ...
    /// Rebuilds the list immutably using [copyWith] to update [priority] fields
    /// after a drag-and-drop reorder. This avoids in-place mutation of the
    /// [SortColumnConfig] objects, which would prevent proper state tracking.
    void applyReorder(int oldIndex, int newIndex) {
      if (oldIndex < newIndex) newIndex -= 1;
      final updated = List<SortColumnConfig>.from(configs.value);
      final moved = updated.removeAt(oldIndex);
      updated.insert(newIndex, moved);
      // Rebuild with new priorities using copyWith — never mutate in-place
      configs.value = [
...
/// Extracted as a private widget to keep the list builder lean.
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
            ...
            // Drag handle — displayed by ReorderableListView automatically
            // but we provide a visual indicator
