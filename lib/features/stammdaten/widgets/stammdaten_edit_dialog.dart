...
class StammdatenEditDialog extends HookConsumerWidget {
  ...
  const StammdatenEditDialog({super.key, this.schluessel});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isLoadingConfig =
        schluessel != null &&
        settingSnapshot.connectionState == ConnectionState.waiting;
    ...
    Future<void> saveSetting() async {
      ...
      try {
        ...
        if (settingSnapshot.data != null) {
          ...
          final fallbackSchluessel = ctrlBezeichnung.text
              .trim()
              .toLowerCase()
          ...
          final newSetting = StammdatenCompanion.insert(
            schluessel:
                ...
                (isSchluesselEmpty
                    ? generatedSchluessel
                    : ctrlSchluessel.text.trim()),
            typ: ctrlTyp.text.trim(), // Default fallback is string
            kategorie: ctrlKategorie.text.isEmpty
                ? 'sonstiges'
                : ctrlKategorie.text.trim(),
            bezeichnung: ctrlBezeichnung.text.trim(),
            wert: drift.Value(ctrlWert.text.trim()),
            beschreibung: drift.Value(ctrlBeschreibung.text.trim()),
        ...
        if (context.mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Einstellung erfolgreich gespeichert'),
            ),
          );
        }
      } catch (e) {
    ...
    return AppEditDialogScaffold(
      ...
      onDelete: (isSystemPflicht.value || schluessel == null)
          ? null
          : () async {
              final repo = ref.read(stammdatenRepositoryProvider);
            ...
            children: [
              Checkbox(
                ...
                onChanged: isReadOnly
                    ? null
                    : (v) => isSystemPflicht.value = v ?? false,
              ),
              const Gap(8),
              const Expanded(
                child: Text(
                  'Zwingend erforderlich (System-Pflicht, darf nicht gelöscht werden)',
                ),
              ),
            ],
          ),
