...
class WarenEditDialog extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final details =
        wareId != null ? detailsList.where((d) => d.ware.id == wareId).firstOrNull : null;
      ...
      if (!isLoadingConfig) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          switch (initialFocusField) {
            case 'bezeichnung': fnBezeichnung.requestFocus();
            case 'kategorie': fnKategorie.requestFocus();
            case 'bestand': fnBestand.requestFocus();
            case 'bruttopreis': fnBruttopreis.requestFocus();
            default: fnBezeichnung.requestFocus(); // sensible default
          }
        });
      }
      return null;
    ...
    Future<void> saveWare() async {
      ...
      isSaving.value = true;
      try {
        await ref.read(warenRepositoryProvider).saveWareFull(
          wareId: wareId,
