...
- **Never `StatefulWidget`** — always `HookConsumerWidget` + `flutter_hooks` (`useTextEditingController()`, etc.)
...
- **Drift table class names** are German-pluralized: `Mitglieds`, `Beitraege`, `Rechnungen`, `RechnungPositionen` (not English)
- **CSV export** uses UTF-8 with BOM for Excel compatibility
- **Dialogs**: Use [`AppEditDialogScaffold`](lib/common_widgets/app_edit_dialog_scaffold.dart:35) — has built-in ESC/Enter shortcuts; Enter skips save when multi-line textfield or dropdown is focused
- **Delete buttons**: Use [`AppDialogDeleteAction`](lib/common_widgets/app_dialog_delete_action.dart:8) — has built-in confirmation
- **2-space indent**, **100-char line length**, **trailing commas on multi-line** (differs from Dart defaults)
- **`@Riverpod(keepAlive: true)`** for global providers like [`appDatabaseProvider`](lib/core/providers/database_provider.dart:11) — all others auto-dispose
